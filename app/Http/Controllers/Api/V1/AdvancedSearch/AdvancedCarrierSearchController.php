 ...
 *
 * 7. DT scoring moved off the request entirely. Live-scoring ten
 *    never-before-seen carriers - loading a dozen relations each, running
 ...
 *    queued if a call site ever needs that), but dispatchScoring() below
 *    deliberately does not push it onto the `dt-score` connection/queue
 *    it declares. afterResponse() runs the job inline, in this same PHP
 *    process, right after the HTTP response has already been flushed to
 *    the broker - so the broker's request is not held up by it, but the
 *    score is not waiting on the next cron-triggered queue:work cycle
 *    either (routes/console.php's scheduled worker only ever needed to
 *    drain `dt-score` because this used to queue there; it no longer
 *    does, so that queue now stays empty on purpose). A poll from the
 *    frontend a few seconds after the search response typically finds a
 *    real score already, not "Calculating..." for up to a minute.
 */
class AdvancedCarrierSearchController extends Controller
{
    ...
    /**
     * DT scores for a set of DOT numbers, polled by the frontend after a
     * search comes back with dt_score: null on some rows.
     *
     * Deliberately just a thin wrapper around scoreCurrentPage() - the same
     * cache-first lookup respondPaginated() already runs, so polling this
     * costs a handful of cache reads, nothing more. A DOT still uncached
     * gets re-dispatched to the background job exactly as a fresh search
     * would, but the dedupe guard in dispatchScoring() means a broker's
     * browser polling every few seconds never queues more than one job per
     * carrier - the second and third polls just find the same "already
     * queued" flag and wait, same as they'd wait on the result either way.
     */
    public function scores(Request $request)
    {
        $request->validate([
            'dots'   => 'required|array|min:1|max:100',
            'dots.*' => 'string|max:20',
        ]);
    ...
    private function hydrateRows(array $rows): array
    {
        ...
        if (!empty($dots)) {
            $census = $this->timed('hydrate: census lookup', fn () => $this->conn()->table('company_census_file')
            ...
            | a list the WHERE has already narrowed. Timed rather than just
            | assumed, though - that assumption is exactly the kind of thing
            | that's cheap right up until an index isn't what it looks like.
            */
            $authority = $this->timed('hydrate: authority lookup', fn () => $this->conn()->table('carrier_authorities as auth_now')
            ...
            $insurance = $this->timed('hydrate: insurance lookup', fn () => $this->conn()->table('actpendinsur_all_with_history')
            ...
            $local = $this->timed('hydrate: local carriers mirror', fn () => DB::table('carriers')
                ->whereIn('dot_number', $dots)
                ->get()
                ->keyBy('dot_number'));
        }
     ...
     * a page with no cached score for a carrier no longer computes it live
     * at all - see the "not yet on the critical path" note below.
     *
    ...
    private function scoreCurrentPage(array $dots): array
    {
        ...
        /*
        | Not on the critical path any more.
        |
        | Live-scoring ten never-before-seen carriers was measured taking
        | 30-90 seconds on its own - inspecting relations, fraud-network
        | queries, running the v3 engine per row - and that whole cost sat
        | between a broker's click and their results, every single time a
        | search page included a carrier nobody had looked at yet. That is
        | the dominant share of the whole request, and it buys the same
        | thing this dispatch buys: the exact same score, in the exact same
        | cache key, just not blocking the response that returns it.
        |
        | Runs ->afterResponse() (see dispatchScoring()), so it starts the
        | moment this response has gone out, not on the next queue:work
        | cron tick. A carrier scored this way shows dt_score: null on
        | THIS response and typically has a real cached score within a
        | few seconds - long before a page reload, usually before the
        | frontend's first poll even lands.
        */
        if (! empty($pending)) {
            $this->dispatchScoring($pending);
        }

        return $dots
            ? $scores + $this->lastKnownScores(array_values(array_diff($dots, array_keys($scores))))
            : $scores;
    }

    /**
     * Schedule background scoring for carriers with no cached score,
     * deduped so a burst of requests touching the same uncached carriers
     * (a broker refreshing, or two teammates searching the same area at
     * once) runs one scoring pass per DOT, not one per request.
     *
     * Dispatched ->afterResponse(): this runs inline, in this same PHP
     * process, right after the HTTP response has already gone out to the
     * broker - not pushed onto the `dt-score` queue for a cron-triggered
     * worker to pick up later. ScoreCarrierSearchPage still implements
     * ShouldQueue so it stays usable as a real queued job if some future
     * call site needs that, but this one deliberately doesn't queue it:
     * a search page's carriers are wanted scored in the next few seconds,
     * not whenever the next queue:work cron tick happens to land.
     *
     * @param  array<int, string>  $dots
     */
    private function dispatchScoring(array $dots): void
    {
        $toDispatch = [];

        foreach ($dots as $dot) {
            // Cache::add() only succeeds if the key was not already set -
            // the same one-lock-per-key pattern CarrierController's own
            // count warm-up uses. Two minutes covers how long scoring
            // itself takes; a request arriving after that is asking again
            // because the first attempt is done or gone, not because it
            // is impatient.
        ...
        try {
            \App\Jobs\ScoreCarrierSearchPage::dispatch($toDispatch)->afterResponse();
        } catch (\Throwable $e) {
            // Registering the after-response callback itself failing
            // (rare - container resolution, not the scoring work, since
            // that hasn't run yet) must not take the search down - the
            // page still renders, just without scores until whatever is
            // wrong here is fixed. A later search naturally retries these
            // DOTs, since the guard below is released either way.
            Log::warning('[CarrierSearch] failed to dispatch background scoring: '.$e->getMessage());

            foreach ($toDispatch as $dot) {
